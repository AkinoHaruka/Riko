...
<#
.SYNOPSIS
    One-click start AI Chat PC (Windows) dev environment (backend + frontend)

.DESCRIPTION
    1. Clean up residual processes on port 3000 (backend)
    2. Start Python FastAPI backend service
    3. Wait for backend health check to pass
    4. Start Flutter Windows desktop app
...
Write-Host "  AI Chat PC Dev Environment Launcher" -ForegroundColor Blue
Write-Host "========================================" -ForegroundColor Blue
Write-Host ""
...
# 4. Start Flutter Windows frontend (foreground, interactive)
Write-Info "Starting Flutter Windows desktop app..."
Write-Host ""
...
function Invoke-Cleanup {
    ...
    # Try to kill Flutter desktop process
    try {
        $flutterProcs = Get-Process -Name "riko" -ErrorAction SilentlyContinue
        if ($flutterProcs) {
            $flutterProcs | Stop-Process -Force
            Write-Success "Flutter desktop process closed"
        }
    } catch {
        # ignore
    }
