...
└── memories/                         # 记忆目录（运行时自动创建）
    ├── MEMORY.md                     # 持久记忆索引（梦境代理维护）
    ├── persistent_memory.md          # 常驻记忆（梦境结果同步）
...
| System | persistent_memory.md | 常驻记忆：跨会话持久化信息 |
...
**流程**：SQLite 查询活跃会话 → 获取锁 → 4 阶段整合 → 同步 persistent_memory.md
